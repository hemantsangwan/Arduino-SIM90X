...
/**********************************************************
 ...
 automatically when this->begin is called (of course
...
char SIMCOM900::forceON() {
    ...
    p_char = strchr((char *) (this->comm_buf), ',');
    p_char1 = p_char + 1;
    *(p_char1 + 2) = 0;
...
int SIMCOM900::configandwait(char* pin) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        while (this->WaitResp(5000, 50, "+CGREG: 0,") != RX_FINISHED_STR_RECV)
...
boolean SIMCOM900::readSMS(char* msg, int msglength, char* number, int nlength) {
#if defined(debug)
    debug.println(F("This method is deprecated! Please use GetSMS in the SMS class."));
#endif
    long index;
    char *p_char;
    char *p_char1;
    ...
    WaitResp(5000, 500);
    if (this->IsStringReceived("+CMGL")) {

        //index
        p_char = strchr((char *) (this->comm_buf), '+CMGL');
...
boolean SIMCOM900::readCall(char* number, int nlength) {
    ...
    //_tf.setTimeout(_GSM_DATA_TOUT_);
    if (this->WaitResp(5000, 50, "+CLIP: \"") != RX_FINISHED_STR_RECV)
            //if(_tf.find("+CLIP: \""))
            {
#ifdef UNO
        _tf.getString("", "\"", number, nlength);
...
int SIMCOM900::setPIN(char *pin) {
    ...
    if (this->WaitResp(5000, 50, str_ok) != RX_FINISHED_STR_NOT_RECV)
        return 0;
    else
    return 1;
}
...
int SIMCOM900::changeNSIPmode(char mode) {
    ...
    SimpleWriteln(mode);
    if (this->WaitResp(5000, 50, str_ok) != RX_FINISHED_STR_NOT_RECV) return 0;
...
/**********************************************************
 ...
 if (1 == this->GetPhoneNumber(1, phone_num)) {
 // valid phone number on SIM pos. #1
 // phone number string is copied to the phone_num array
 #ifdef DEBUG_PRINT
 this->DebugPrint("DEBUG phone number: ", 0);
 this->DebugPrint(phone_num, 1);
 #endif
 }
 else {
 // there is not valid phone number on the SIM pos.#1
 #ifdef DEBUG_PRINT
 this->DebugPrint("DEBUG there is no phone number", 1);
 #endif
 }
 **********************************************************/
...
/**********************************************************
 ...
 an example of usage:
 if (1 == this->ComparePhoneNumber(1, "123456789")) {
 // the phone num. "123456789" is stored on the SIM pos. #1
 // phone number string is copied to the phone_num array
 #ifdef DEBUG_PRINT
 this->DebugPrint("DEBUG phone numbers are the same", 1);
 #endif
 }
 else {
 #ifdef DEBUG_PRINT
 this->DebugPrint("DEBUG phone numbers are different", 1);
 #endif
 }
 **********************************************************/
