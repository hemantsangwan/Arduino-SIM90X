...
int InetGSM::httpPOST(const char* server, int port, const char* path, const char* parameters, char* result,
        int resultlength)
        {
    boolean connected = false;
    int n_of_at = 0;
...
int InetGSM::attachGPRS(char* domain, char* dom1, char* dom2)
    ...
    if (gsm.WaitResp(5000, 50, "ERROR") != RX_FINISHED_STR_RECV) {
#ifdef debug
        debug.println("DB:ALREADY HAVE AN IP");
#endif
...
#ifdef debug
        debug.println("DB:STARTING NEW CONNECTION");
#endif
...
#ifdef debug
        debug.println("DB:CONNECTION OK");
#endif
        ...
        if (gsm.WaitResp(5000, 50, "ERROR") != RX_FINISHED_STR_RECV) {
#ifdef debug
            debug.println("DB:ASSIGNED AN IP");
#endif
            gsm.setStatus(gsm.ATTACHED);
            return 1;
        }
#ifdef debug
        debug.println("DB:NO IP AFTER CONNECTION");
#endif
        return 0;
    }
}
...
int InetGSM::dettachGPRS()
{
    ...
    //Stop IP stack.
    //gsmSerial << "AT+WIPCFG=0" <<  _DEC(cr) << endl;
    //	if(!gsm._tf.find("OK")) return 0;
    //Close GPRS bearer.
    //gsmSerial << "AT+WIPBR=0,6" <<  _DEC(cr) << endl;
