...
#ifdef debug
            debug.println(F("DB:AUTO BAUD RATE"));
#endif
        for (int i = 0; i < 8; i++) {
            switch (i) {
            case 0:
...
#if defined(debug)
        debug.println(F("Trying to force the baud-rate to 9600\n"));
#endif
        for (int i = 0; i < 8; i++) {
            switch (i) {
            case 0:
...
#if defined(debug)
        debug.println(F("ERROR: SIM900 doesn't answer. Check power and serial pins in GSM.cpp"));
#endif
        digitalWrite(GSM_ON, HIGH);
        delay(1200);
        digitalWrite(GSM_ON, LOW);
...
byte GSM::IsStringReceived(char const *compare_string)
    ...
    if (comm_buf_len) {
...
#ifdef debug
        debug.print("ATT: ");
        debug.println(compare_string);
        debug.print("RIC: ");
        debug.println((char *) comm_buf);
#endif
        ...
        debug.print(F("ATT: "));
        debug.println(compare_string);
        debug.print(F("RIC: NO STRING RCVD"));
#endif
    }
